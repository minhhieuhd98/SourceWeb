	...
	if($_POST['password']==$_POST['passverif'])
	{
		if(strlen($_POST['password'])>=6)
		{
			if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
			{
				...
				$dn = mysqli_num_rows(mysqli_query($connect, 'select username from users where username="'.$username.'"'));

				if($dn==0)
				{
					$dn2 = mysqli_num_rows(mysqli_query($connect, 'select id from users'));
					$id = $dn2+1;
					if(mysqli_query($connect, 'insert into users(id, username, password, email, avatar, signup_date) values ('.$id.', "'.$username.'", "'.$password.'", "'.$email.'", "'.$avatar.'", "'.time().'")'))
					{
						$form = false;
?>
