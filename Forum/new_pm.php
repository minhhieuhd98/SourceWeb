...
if(isset($_POST['title'], $_POST['recip'], $_POST['message']))
{
	...
	{
		$title = mysqli_real_escape_string($connect, $otitle);
		$recip = mysqli_real_escape_string($connect, $orecip);
		$message = mysqli_real_escape_string($connect, nl2br(htmlentities($omessage, ENT_QUOTES, 'UTF-8')));
		$dn1 = mysqli_fetch_array(mysqli_query($connect, 'select count(id) as recip, id as recipid, (select count(*) from pm) as npm from users where username="'.$recip.'"'));
		if($dn1['recip']==1)
		{
			if($dn1['recipid']!=$_SESSION['userid'])
			{
				$id = $dn1['npm']+1;
				if(mysqli_query($connect, 'insert into pm (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "1", "'.$title.'", "'.$_SESSION['userid'].'", "'.$dn1['recipid'].'", "'.$message.'", "'.time().'", "yes", "no")'))
				{
	?>
